view: a_sum_m_copy {
  derived_table: {
    sql: with true_users as (
      ...
      , res as (
      select
      case when _start <= days.day and _end >= days.day and _all.status like 'Trial access users' then 'Trial access users'
      when _start <= days.day and _end >= days.day and _all.status like 'Full Access - Direct Purchase' then 'Full Access - Direct Purchase'
      when _start <= days.day and _end >= days.day and _all.status like 'Full Access - Upgraded' then 'Full Access - Upgraded'
      when _end < days.day then 'z_Expeired' end as active_status
      , days.day
      , _all.*
      from _all, days
      )
