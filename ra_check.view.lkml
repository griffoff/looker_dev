view: ra_check {
 derived_table: {
  sql: select
      ...
      from prod.UNLIMITED.SCENARIO_ACTIVITIES a
      inner join prod.UNLIMITED.SCENARIO_ENROLLMENTS e on e.uid = a.uid
      inner join prod.UNLIMITED.SCENARIO_PROVISIONING p on p.uid = a.uid
      inner join prod.UNLIMITED.SCENARIO_SUBSCRIPTIONS s on s.uid = a.uid
      inner join prod.UNLIMITED.SCENARIO_DETAILS as plan on plan.uid = a.uid
       ;;
}
